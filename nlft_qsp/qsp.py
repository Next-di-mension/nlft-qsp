...
class PhaseFactors:
    ...
    def conjugation(self, P, Q):
        """Given (P, Q), this applies a final conjugation, e.g., with the Hadamard gate."""
        return P, Q
...
class ChebyshevQSPPhaseFactors(XQSPPhaseFactors):
    ...
    def conjugation(self, P, Q): # Return the polynomials (P', Q') = H (P, Q) H
        return ((P + P.conjugate()) + (Q - Q.conjugate()))/2, ((P - P.conjugate()) - (Q + Q.conjugate()))/2


def nlfs_to_phase_factors(F: NonLinearFourierSequence, alpha: generic_real = 0) -> GQSPPhaseFactors:
